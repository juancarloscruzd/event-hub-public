describe("evetUtils", function() {
    ...
    it("should check if the given event has a date", function() {
        expect(eventUtils.checkEventDate({eventDate: new Date().getTime()})).toBeTruthy();
    });
    ...
    it("should check if the given event is indeed an event", function() {
        expect(eventUtils.isEvent({eventDate:new Date().getTime(), eventType:'MyType'})).toBeTruthy();
    });
    it("should check if the given event is indeed an event (false)", function() {
        expect(eventUtils.isEvent({eventDate:new Date().getTime(), eventType:''})).toBeFalsy();
    });
    // Stringfy
    it("should convert an event into a json string", function() {
        let time = new Date().getTime();
        expect(eventUtils.stringify({eventDate:time, eventType:'MyType'})).toBe("{\"eventDate\":"+time+",\"eventType\":\"MyType\"}");
    });
    // GetOriginal
    ...
    it("should return the original event, contained on the given object - Message", function() {
        ...
        expect(eventUtils.getOriginal({Message:"{\"eventDate\":"+time+",\"eventType\":\"MyType\"}"})).toEqual(event);
    });
